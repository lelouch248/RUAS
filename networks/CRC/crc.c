...
void remCal()
{
    /*creates a copy of the data of the size of the poly_len*/
    ...
    do
    {
        if (rem[0] == '1')
            xor();
        /*new remainder calculation*/

        for (c = 0; c < poly_len - 1; c++)
            rem[c] = rem[c + 1];

        /* an additionl bit is takken from the neighbout just to statisfy the length of the data 
        with the gen_poly */
...
void sender()
{
    ...
    /*adding the additional 0 bits into the data*/
    ...
    remCal();
    printf("\n\nremainder is: %s", rem);
    /*adding the remainder bits to the end of the sender data*/
    for (i = data_len; i < data_len + poly_len - 1; i++)
...
{
    remCal();
    for (i = 0; (i < poly_len - 1) && (rem[1] != '1'); i++)
        ;
    if (i < poly_len - 1)
